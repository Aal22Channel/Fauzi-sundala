...
let ppUrl = await conn.profilePictureUrl(who, 'image').catch((_) => "https://telegra.ph/file/c60fb27e4898462f98aa8.jpg");
  ...
  let ucpn = `${ucapan()}`
    readmore: readMore
  let menyu = `
︎︎ 𝙃𝙖𝙞 𝙠𝙖𝙠 ${name} 𝙨𝙖𝙮𝙖 𝙖𝙙𝙖𝙡𝙖𝙝 𝙒𝙖𝙗𝙤𝙩 𝙮𝙖𝙣𝙜 𝙙𝙞𝙗𝙪𝙖𝙩 𝙤𝙡𝙚𝙝 𝑭𝒂𝒓𝒊𝒅 𝑴𝒐𝒅𝒛. 𝙖𝙙𝙖 𝙮𝙖𝙣𝙜 𝙗𝙞𝙨𝙖 𝙙𝙞 𝙗𝙖𝙣𝙩𝙪?シ︎.

╭─〢「 *𝙐𝙎𝙀𝙍 𝙄𝙉𝙁𝙊* 」︎
│𝕱•➤︎ 𝗡𝗮𝗺𝗲 : ${name}
│𝕱•➤ 𝗥𝘂𝗻𝘁𝗶𝗺𝗲 : ${uptime}
│𝕱•➤ 𝗠𝗼𝗱𝗲 : ${global.opts['self'] ? 'Self' : 'Publik'}
├─〢「 *𝘽𝙊𝙏 𝙄𝙉𝙁𝙊* 」
│𝕱•➤ 𝗢𝘄𝗻𝗲𝗿 : @𝙵𝙰𝚁𝙸𝙳☂︎
│𝕱•➤ 𝗖𝗿𝗲𝗮𝘁𝗼𝗿 : ${global.author}
│𝕱•➤ 𝗨𝘀𝗲𝗿 : ${Object.keys(global.db.data.users).length}
│𝕱•➤ 𝗗𝗮𝘁𝗲 : ${week}, ${date}
│𝕱•➤ 𝗧𝗶𝗺𝗲 : ${time}
╰────────────⬣᭄
